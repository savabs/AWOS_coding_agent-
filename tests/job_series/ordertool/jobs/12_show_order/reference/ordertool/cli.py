...
from .reports import render_order, render_summary
...
def build_parser():
    ...
    imp = sub.add_parser("import", help="add orders from a CSV file")
    imp.add_argument("file", help="CSV with customer,email,created_at,items[,discount] columns")
    ...
    status.add_argument("order_ids", nargs="+", metavar="ORDER_ID", help="one or more order ids")
    ...
    monthly = sub.add_parser("monthly", help="export per-month totals as CSV")
    monthly.add_argument("--output", "-o", help="write to this file instead of stdout")
    add_date_range_args(monthly)
    monthly.set_defaults(handler=cmd_monthly)

    products = sub.add_parser("products", help="export per-product totals as CSV")
    products.add_argument("--output", "-o", help="write to this file instead of stdout")
    add_date_range_args(products)
    products.set_defaults(handler=cmd_products)

    show = sub.add_parser("show", help="show one order in detail")
...
def cmd_import(service, args, out):
    """Handle ``import``."""
    orders = service.import_orders(read_import_file(args.file))
    print(f"imported {len(orders)} orders", file=out)
    return 0
...
def cmd_set_status(service, args, out):
    """Handle ``set-status`` (one or more ids; all change or none do)."""
    for order in service.set_status_many(args.order_ids, args.status):
...
def cmd_monthly(service, args, out):
    """Handle ``monthly``."""
    date_from, date_to = date_range(args)
    orders = filter_orders(service.list_orders(), date_from=date_from, date_to=date_to)
    if args.output:
        count = write_output_file(args.output, lambda fh: export_monthly_csv(orders, fh))
        print(f"exported {count} rows to {args.output}", file=sys.stderr)
    else:
        export_monthly_csv(orders, out)
    return 0


def cmd_products(service, args, out):
    """Handle ``products``."""
    date_from, date_to = date_range(args)
    orders = filter_orders(service.list_orders(), date_from=date_from, date_to=date_to)
    if args.output:
        count = write_output_file(args.output, lambda fh: export_products_csv(orders, fh))
        print(f"exported {count} rows to {args.output}", file=sys.stderr)
    else:
        export_products_csv(orders, out)
    return 0


def cmd_show(service, args, out):
    """Handle ``show``."""
    print(render_order(service.get_order(args.order_id)), file=out)
    return 0
