...
from .utils.dates import format_day, format_timestamp
...
def render_order(order):
    """Render one order in detail (used by ``ordertool show``)."""
    lines = [
        f"{order.id}  {order.customer} <{order.email}>",
        f"status: {order.status.value}",
        f"created: {format_timestamp(order.created_at)}",
    ]
    for item in order.items:
        lines.append(
            f"{item.sku} x{item.quantity} @ {format_money(item.unit_price)} = {format_money(item.subtotal)}"
        )
    lines.append(f"subtotal: {format_money(order.gross_total)}")
    if order.discount_pct:
        pct = format(order.discount_pct.normalize(), "f")
        lines.append(f"discount: {pct}% -{format_money(order.gross_total - order.total)}")
