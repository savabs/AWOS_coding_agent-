...
DATE_FORMAT = "%Y-%m-%d"
# Everything parse_timestamp accepts, most specific first. Output always uses
# TIMESTAMP_FORMAT.
INPUT_FORMATS = (
    TIMESTAMP_FORMAT,
    "%Y-%m-%d %H:%M:%S",
...
def parse_timestamp(text):
    ...
    Accepts the full storage format and, for convenience with hand-written
    input, a space instead of the ``T``, a time without seconds, or a bare
    ``YYYY-MM-DD`` date (interpreted as midnight).
