...
def build_monthly_summary(store, fx, categoriser, account_id, year, month):
    ...
    merchant_spend = {}
    largest = None
    month_settled = [_settled(fx, txn, base) for txn in month_txns]
    for txn, settled in zip(month_txns, month_settled):
        if settled >= 0:
            income += settled
        else:
            spending += -settled
    ...
    # End-of-day balances: one running total over the (date-ordered) month.
    movement_by_day = {}
    for txn, settled in zip(month_txns, month_settled):
        movement_by_day[txn.posted_on] = movement_by_day.get(txn.posted_on, ZERO) + settled
    balance = opening
    day = first
    while day <= last:
        movement = movement_by_day.get(day)
        if movement is not None:
            balance += movement
        summary.daily_balances.append((day, balance))
