...
def local_date(value: datetime, tz_name: str = "UTC") -> date:
    """The calendar day on which ``value`` (naive UTC or aware) falls in ``tz_name``."""
    return to_local(value, tz_name).date()


def day_bounds(day: date, tz_name: str = "UTC") -> Tuple[datetime, datetime]:
    """Return the half-open ``[start, end)`` range covering ``day`` in ``tz_name``.

    ``day`` is a calendar day in the customer's zone. The bounds are the local
    midnights that open and close it, converted to naive UTC so they can be
    compared directly with stored timestamps. Days around DST transitions are
    23 or 25 hours long, so ``end`` comes from the next local midnight rather
    than from adding 24 hours.
    """
...
def today(clock: Clock, tz_name: str = "UTC") -> date:
    """The current calendar day in ``tz_name`` according to ``clock``."""
    return clock.now().astimezone(get_zone(tz_name)).date()
