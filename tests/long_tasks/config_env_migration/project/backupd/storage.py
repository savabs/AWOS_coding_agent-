...
def _read_storage_settings(settings_path=DEFAULT_SETTINGS_PATH):
    # Storage only needs two values, so it reads them straight from the
    # settings file instead of paying for a full load_config().
    ...
    try:
        retention = parser.getint("backup", "retention_days", fallback=7)
    except ValueError as exc:
        raise ConfigError("setting 'retention_days' must be an integer") from exc
    return Path(raw_dir).expanduser(), retention
